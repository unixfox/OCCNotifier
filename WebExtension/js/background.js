			...
			if (isSettingSet('region', serverData.region)) {
				...
				if (!isPlaying(serverId)) {
					...
					if (isSettingSet('notification', 'maps')) {
						...
						createNotification(serverData.map.name, serverData.name, serverData.region, serverData.participants, serverData.maxPlayers, serverId, serverData.tags);
...
function createNotification(mapName, serverName, serverRegion, serverParticipants, serverMaxPlayers, serverId, serverTags) {
	...
	var opt = {
		...
		message: 'Players: ' + serverParticipants + '/' + serverMaxPlayers + "\n" + tagsMessage,
		iconUrl: "https://raw.githubusercontent.com/OvercastCommunity/maps/master/" + mapName + "/map.png"
	};
